"""Real YouTube evidence collection for us_hvac_10_99 M2 commercial pain.

Directive-required pipeline, phase 1 of 2 (discovery -> retrieval -> raw
evidence retention): search a deliberately diverse set of real HVAC
dispatch/scheduling/operations queries, then fetch real comment threads
for every distinct video found. Writes raw evidence only -- no
extraction, no scoring. Reuses the existing youtube_data adapter
completely unchanged.

Each retained YouTube comment is treated as one distinct document for
M2's evidence-count purposes (a real, individually-authored public
statement, not a fragment of a larger page) -- see the extraction script
for why this granularity was chosen over "one document per video".

Usage:
    uv run python scripts/collect_youtube_pain_evidence_us_hvac_10_99.py
"""
...
# Deliberately diverse real queries covering the market config's seed_terms
# and seed_problems (configs/markets/us_hvac_10_99.yaml), not one narrow
# phrase repeated -- source-family/topic diversity matters for M2 evidence.
SEARCH_QUERIES = [
    "HVAC dispatch software",
    "HVAC scheduling software problems",
    "HVAC field service management software",
    "HVAC company missed calls leads",
    "HVAC dispatcher day in the life",
    "HVAC business scheduling nightmare",
    "HVAC technician scheduling app",
    "HVAC estimating software small business",
    "running an HVAC business operations",
    "HVAC service call management",
    "HVAC customer service complaints",
    "HVAC software review small business",
    "HVAC business owner problems",
]
RESULTS_PER_QUERY = 10
MAX_COMMENTS_PER_VIDEO = 100
RUN_ID = f"youtube_pain_evidence_us_hvac_10_99_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}"
...
def main() -> None:
    ...
    for video_id, video in videos.items():
        try:
            ...
            # commentsDisabled / not found are normal, expected outcomes for
            # some videos, not evidence-pipeline failures -- recorded, not fatal.
