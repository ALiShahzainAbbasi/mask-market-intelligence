...
- Current local checks: all 5 official-data live smokes pass; the complete offline suite passes with 580 Python and 11 web tests, including A09-A17 extraction, grounding, M2 pain intelligence, M3 workflow-bottleneck calculation, the M1/M4/M5/M6/M7/M9 formula calculators, the confidence/completeness/automatic-veto engine, the automated-research-score/gate/snapshot engine, the JSON/HTML/CSV report generator, and the new A17.5 Gemini provider suite (schema-dialect translation, prompt-injection-in-source-text containment, usage/cost parsing, and refusal/incomplete/cache behavior, all against a fake transport). The static gates are recorded in `progress.txt`. Supabase acceptance remains recorded there; no paid research ran.
