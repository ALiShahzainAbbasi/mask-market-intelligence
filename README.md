...
- Current local checks: all 5 original official-data live smokes pass, plus a new live Common Crawl smoke (real CDX lookup + ranged WARC fetch against `example.com`, passed). USAspending's equivalent live smoke test exists but has not yet been run. O*NET's live download was attempted twice against the real server: the first attempt exposed and led to fixing a real silent-truncation bug, the second (post-fix) correctly caught a genuine mid-transfer connection drop as a clean error instead of corrupting the cache -- but neither completed a full pass, an honest, documented limitation of this environment's connection, not a known code defect (see `docs/ONET_DATA.md`). The complete offline suite passes with 696 Python and 11 web tests, including A09-A17 extraction, grounding, M2 pain intelligence, M3 workflow-bottleneck calculation, the M1/M4/M5/M6/M7/M9 formula calculators, the confidence/completeness/automatic-veto engine, the automated-research-score/gate/snapshot engine, the JSON/HTML/CSV report generator, and the new A17.5 Gemini, YouTube, USAspending, O*NET-bootstrap, and Common-Crawl-retrieval suites (schema-dialect translation, prompt-injection containment, usage/cost parsing, quota-unit accounting, distinct quota/error paths, keyless-request validation, cache-hit/redownload logic, CSV parsing against real-verified column headers, and CDX/WARC parsing against a real-verified record structure, all against fake transports or fixtures). The static gates are recorded in `progress.txt`. Supabase acceptance remains recorded there; no paid research ran.
...
- Next code: the free-source real-data expansion (A17.5) -- live Gemini/YouTube providers, USAspending, an O*NET dataset importer, a targeted Common Crawl retriever, a source-fallback registry, and the first real `MethodExecutor` wiring -- aimed at one real, source-backed market report at $0 paid-data cost. Google Ads, Reddit, Google Places, Meta, Brave, and firmographic providers remain held/optional. No paid provider, outreach, campaign, or deployment was activated.
