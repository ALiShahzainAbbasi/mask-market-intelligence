...
- Current local checks: all 5 official-data live smokes pass; the complete offline suite passes with 619 Python and 11 web tests, including A09-A17 extraction, grounding, M2 pain intelligence, M3 workflow-bottleneck calculation, the M1/M4/M5/M6/M7/M9 formula calculators, the confidence/completeness/automatic-veto engine, the automated-research-score/gate/snapshot engine, the JSON/HTML/CSV report generator, and the new A17.5 Gemini and YouTube provider suites (schema-dialect translation, prompt-injection containment, usage/cost parsing, quota-unit accounting, the distinct quota-exceeded error path, and refusal/incomplete/cache behavior, all against fake transports). The static gates are recorded in `progress.txt`. Supabase acceptance remains recorded there; no paid research ran.
