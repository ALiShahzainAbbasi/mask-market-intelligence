...
- Autonomous functional track: A01-A05, A10-A17 are complete. The fixed-scope Census CBP, keyless BLS, BEA Regional, SEC EDGAR, and SAM.gov adapters passed one deliberately small live check each; A17.5 adds a sixth, keyless official_data source (USAspending award search for M4/M5), offline-tested only so far. Gemini and YouTube Data API credentials are now configured locally (owner-supplied); live wiring for both is in progress under the free-source real-data expansion (A17.5). A06/A07 remain partial while other providers stay on hold; A09's offline structured-analysis boundary is implemented, and A17.5 has now added a Gemini `generateContent` provider mirroring it exactly, plus a separate `youtube_data` module for `search.list`/`commentThreads.list` with its own quota-unit accounting (both offline-tested against fake transports only; no live call made for either, and both stay disabled until explicit policy approval).
...
- Current local checks: all 5 original official-data live smokes pass (the new sixth, USAspending, has an equivalent live smoke test that has not yet been run); the complete offline suite passes with 628 Python and 11 web tests, including A09-A17 extraction, grounding, M2 pain intelligence, M3 workflow-bottleneck calculation, the M1/M4/M5/M6/M7/M9 formula calculators, the confidence/completeness/automatic-veto engine, the automated-research-score/gate/snapshot engine, the JSON/HTML/CSV report generator, and the new A17.5 Gemini, YouTube, and USAspending provider suites (schema-dialect translation, prompt-injection containment, usage/cost parsing, quota-unit accounting, the distinct quota-exceeded error path, keyless-request validation, and refusal/incomplete/cache behavior, all against fake transports or fixtures). The static gates are recorded in `progress.txt`. Supabase acceptance remains recorded there; no paid research ran.
