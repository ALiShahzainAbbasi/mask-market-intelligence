...
- modules/method_metrics/: typed, source-grounded metric/provenance contracts and the exact formula-v1 M1/M4/M5/M6/M7 source-to-metric transforms and calculators. It turns already-normalized values from official_data/search_intent/manual capture into scores; it does not fetch, discover, or persist evidence, and it never renormalizes a weighted sum around a missing component.
