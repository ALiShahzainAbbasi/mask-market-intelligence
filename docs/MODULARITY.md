...
- modules/reporting/: a pure presentation layer over `market_scoring`'s `MarketScoreSnapshot` -- the machine-readable JSON report, a self-contained static HTML report (every interpolated value HTML-escaped), and the CSV source manifest. It performs no calculation and never reshapes a method result's internals, only carries it through; a future LLM narrative-summarization layer may only consume its already-validated fields, never alter a score/confidence/gate/veto.
