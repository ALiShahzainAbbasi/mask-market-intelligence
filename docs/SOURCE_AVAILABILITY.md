...
| O*NET downloadable database | Official keyless static dataset | M3 occupation/task mapping | Available; keyless; added under A17.5; offline suite passes, live download attempted twice, not yet fully completed (see `docs/ONET_DATA.md`) |
| Common Crawl (targeted CDX + ranged WARC) | Public keyless historical archive | Historical/fallback evidence for an already-approved URL | Available; keyless; added under A17.5; live CDX lookup + ranged WARC fetch verified against `example.com` |
